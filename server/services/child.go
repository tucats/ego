...
func callChildServices(session *server.Session, w http.ResponseWriter, r *http.Request) int {
	...
	ui.Log(ui.ChildLogger, "[%d] Child service invocation, %s %s", child.SessionID, child.Method, child.Path)
	...
	if err != nil {
		ui.Log(ui.ChildLogger, "[%d] Child service JSON marshalling error: %s", child.SessionID, err.Error())
	...
	if err != nil {
		ui.Log(ui.ChildLogger, "[%d] Child service request write error: %s", child.SessionID, err.Error())
	...
	ui.Log(ui.ChildLogger, "[%d] Child service invoked with: %s", child.SessionID, strings.Join(strArray, " "))
	...
	b, err = cmd.Output()
	if err != nil {
		ui.Log(ui.ServerLogger, "[%d] Child service execution error: %s", child.SessionID, err.Error())
	...
	if err != nil {
		ui.Log(ui.ChildLogger, "[%d] Child service response read error: %s", child.SessionID, err.Error())
	...
	if err != nil {
		ui.Log(ui.ChildLogger, "[%d] Child service invalid response: %s", child.SessionID, err.Error())
	...
	if settings.GetBool(defs.ChildRequestRetainSetting) {
		ui.Log(ui.ChildLogger, "[%d] Retaining child service request file %s", child.SessionID, requestFileName)
		ui.Log(ui.ChildLogger, "[%d] Retaining child service response file %s", child.SessionID, responseFileName)
	} else {
		_ = os.Remove(requestFileName)
		ui.Log(ui.ChildLogger, "[%d] Deleted child service request file %s", child.SessionID, requestFileName)

		_ = os.Remove(responseFileName)
		ui.Log(ui.ChildLogger, "[%d] Deleted child service response file %s", child.SessionID, responseFileName)
	}
...
func ChildService(filename string) error {
	...
	ui.Log(ui.ChildLogger, "[%d] Child service running as PID %d", r.SessionID, os.Getpid())
...
func waitForTurn(id int) (bool, error) {
	...
	ui.Log(ui.ChildLogger, "[%d] Waiting for child service slot (%d currently active)", id, active)
	...
	for {
		...
		if time.Now().After(timeout) {
			ui.Log(ui.ChildLogger, "[%d] Child service timeout waiting for slot", id)

...
func callChildServices(session *server.Session, w http.ResponseWriter, r *http.Request) int {
	...
	// that file.
	filename := filepath.Join(ChildTempDir, fmt.Sprintf("ego-request-%s-%d.json", child.ServerID, child.SessionID))
	...
	strArray := fork.MungeArguments(os.Args[0], "--service", filename)
	cmd := exec.Command(strArray[0], strArray[1:]...)

	b, err = cmd.Output()
	if err != nil {
		ui.Log(ui.ServerLogger, "[%d] Child service error: %s", child.SessionID, err.Error())
	...
	if err != nil {
		ui.Log(ui.ServerLogger, "[%d] Child service invalid response: %s", child.SessionID, err.Error())
	...
	if settings.GetBool(defs.ChildRequestRetainSetting) {
		ui.Log(ui.ServerLogger, "[%d] Retaining child service request file %s", child.SessionID, filename)
	} else {
...
func ChildService(filename string) error {
	...
	serviceCode, tokens, err := compileChildService(r.SessionID, endpoint, r.Filename, symbolTable)
	if err != nil {
		ui.Log(ui.ServicesLogger, "[%d] compilation error, %v", r.SessionID, err.Error())
	...
	if debug {
		ui.Log(ui.ServicesLogger, "Debugging started for service %s %s", r.Method, r.Path)
		ctx.SetTokenizer(tokens)

		err = debugger.Run(ctx.SetTokenizer(tokens))

		ui.Log(ui.ServicesLogger, "Debugging ended for service %s %s", r.Method, r.Path)
	} else {
	...
	if err != nil {
		...
		ui.Log(ui.ServicesLogger, "[%d] Service execution error: %v", r.SessionID, err)
	}
...
func waitForTurn(id int) bool {
	...
	ui.Log(ui.ServerLogger, "[%d] Waiting for child service slot (%d active)", id, active)

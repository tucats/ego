...
func TestComparisons(t *testing.T) {
	tests := []struct {
		...
		{
			name: "array equality",
			v1:   data.NewArrayFromList(data.IntType, data.NewList(5, 2, 6)),
			v2:   data.NewArrayFromList(data.IntType, data.NewList(5, 2, 6)),
			r:    true,
			f:    equalByteCode, i: nil, err: false,
		},
		{
			...
			v1:   data.NewArrayFromList(data.IntType, data.NewList(5, 2, 6)),
			v2:   data.NewArrayFromList(data.Float64Type, data.NewList(5, 2, 6)),
			r:    false,
			f:    equalByteCode, i: nil, err: false,
		},
		{
			...
			v1:   data.NewArrayFromList(data.IntType, data.NewList(5, 2, 6)),
			v2:   data.NewArrayFromList(data.IntType, data.NewList(5, 6, 2)),
			r:    false,
			f:    equalByteCode, i: nil, err: false,
		},
	}

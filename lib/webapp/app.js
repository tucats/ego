...
const consoleDivider   = document.getElementById('console-divider');   // Horizontal drag handle above the console
const consolePane      = document.getElementById('console-pane');      // The console section element
const consoleHistory   = document.getElementById('console-history');   // Scrollable output history area
const consoleInput     = document.getElementById('console-input');     // The single-line text input
const clearConsoleBtn  = document.getElementById('clear-console-btn'); // ✕ button on the console label
...
editor.addEventListener('keydown', e => {
  ...
  if (e.key === 'Tab') {
    ...
    // Programmatically setting editor.value does not fire the 'input' event,
    // so the highlight layer must be updated manually here.
    updateHighlight();
  }
});

// ---------------------------------------------------------------------------
// Console pane — vertical resize
// ---------------------------------------------------------------------------
...
consoleDivider.addEventListener('mousedown', e => {
  ...
  const startHeight = consolePane.getBoundingClientRect().height;

  function onMouseMove(e) {
    // Dragging up increases console height; dragging down decreases it.
    const newHeight = Math.max(60, startHeight - (e.clientY - startY));
    consolePane.style.flexBasis = newHeight + 'px';
  }
...
// ---------------------------------------------------------------------------
// Console pane — REPL logic
// ---------------------------------------------------------------------------

// Append a command + its output (or error) to the scrollable history area.
function consoleAppend(code, outputText, isError) {
  const entry = document.createElement('div');
  entry.className = 'console-entry';

  // "ego> <code>" line in accent purple.
  const cmdLine = document.createElement('div');
  cmdLine.className = 'console-cmd';
  cmdLine.textContent = 'ego> ' + code;
  entry.appendChild(cmdLine);

  // Output block — only shown when the server returned something.
  if (outputText) {
    const outLine = document.createElement('div');
    outLine.className = isError ? 'console-err' : 'console-out';
  ...
  // Scroll to the bottom so the most recent result is always visible.
  consoleHistory.scrollTop = consoleHistory.scrollHeight;
}

// Send the current input line to the server and display the result.
async function runConsole() {
  const code = consoleInput.value;
  if (!code.trim()) return;   // ignore empty input
  consoleInput.value = '';    // clear input before the await so it feels instant
...
// Run on Enter; ignore modifier keys so Shift+Enter etc. can be added later.

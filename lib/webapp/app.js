...
// Close the menu whenever the user clicks anywhere outside it.
...
// ✕ on the editor label — wipe the editor and resync line numbers.
...
// Keep line numbers up to date as the user types or pastes.
editor.addEventListener('input',  updateLineNumbers);

// Keep the gutter scrolled in sync when the user scrolls the editor.
editor.addEventListener('scroll', () => { lineNumbers.scrollTop = editor.scrollTop; });

// Populate line numbers immediately so they're present on first paint.
updateLineNumbers();
...
async function runCode() {
  ...
  try {
    ...
    // Always re-enable the Run button and hide the spinner, even on error.

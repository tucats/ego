...
async function loadLog() {
    ...
    try {
        ...
        // The server returns plain text when the Accept header is text/plain.
        ...
        // res.text() returns the entire response body as a plain string.
        const text = await res.text();

        // Wrap in a <pre> so newlines and spacing are preserved exactly.
        // escapeHtml() prevents any log content from being interpreted as HTML.
        const pre = document.createElement('pre');
        pre.textContent = text;  // textContent is safe — no HTML parsing
        container.innerHTML = '';
        container.appendChild(pre);

        // Scroll to the bottom so the most recent (last) lines are visible.

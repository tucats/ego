...
// Cookie helpers
//
// The dashboard uses a browser cookie named "ego_token" to store the
// bearer token returned by the server after a successful login. Cookies
// are small pieces of text the browser stores and sends back automatically
// with every request to the same site.
// ==========================================================================

// Read the ego_token cookie and return its value.
// document.cookie is a single string containing all cookies separated by
// semicolons, e.g. "ego_token=abc123; other=xyz". The regular expression
// searches that string for our cookie name and captures its value.
// Returns null if the cookie is not present (i.e. the user is not logged in).
function getToken() {
    const match = document.cookie.match(/(?:^|;\s*)ego_token=([^;]+)/);
    return match ? decodeURIComponent(match[1]) : null;
}

// Save the bearer token in a session cookie so it persists across page
// page changes within the same browser tab. The cookie is cleared
// automatically when the tab is closed because we don't set an expiry date.
// encodeURIComponent makes the value safe to store in a cookie by escaping
// special characters (spaces, equals signs, etc.).
// path=/ means the cookie is sent for every URL on this site.
// SameSite=Strict prevents the cookie from being sent on cross-site requests,
// which protects against CSRF attacks.
function setToken(token) {
    document.cookie = 'ego_token=' + encodeURIComponent(token) + '; path=/; SameSite=Strict';
}

// Delete the token cookie by setting its max-age to 0, which tells the
// browser to discard it immediately. Called when the server rejects our
// token so the user is prompted to log in again.
function clearToken() {
    document.cookie = 'ego_token=; path=/; max-age=0; SameSite=Strict';
}
...
// returned token in a cookie and reload the active tab. On failure, show
...
async function submitLogin() {
    ...
    try {
        ...
        // Success — persist the token and proceed to the dashboard.
        setToken(data.token);
        hideLogin();
...
// If no token cookie exists the user hasn't logged in yet, so show the
// login overlay right away. Either way, call openTab() to highlight the
// default Memory tab and set up the tab bar appearance.
if (!getToken()) {
    showLogin();
    // Still open the default tab so the tab bar looks right behind the overlay.
    openTab('memory');
} else {
    openTab('memory');
}

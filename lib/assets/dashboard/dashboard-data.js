...
async function loadData() {
    ...
    try {
        ...
        if (e.status === 403) {
            document.getElementById('data-content').innerHTML =
                '<p style="padding:1rem;color:#c0392b;">' + escapeHtml(e.message) + '</p>';
        } else if (e.message !== 'Unauthorized') {
            console.error('Error loading DSNs for Data tab:', e);
        }
        return;
    }
    ...
    try {
        ...
        if (!res.ok) {
            ...
            container.innerHTML = '<p style="padding:1rem;color:#c0392b;">'
                + escapeHtml(data.msg || 'Failed to load tables (HTTP ' + res.status + ').') + '</p>';
            return;
        }
        ...
        if (e.status === 403) {
            ...
            console.error('Error loading tables for Data tab:', e);
        }
        return;
    }
...
async function loadDataMeta() {
    ...
    try {
        ...
        if (e.message !== 'Unauthorized' && e.status !== 403) console.error('Error loading column metadata:', e);
        _dataColumnMeta = [];
    }
...
// ==========================================================================
// Row edit sheet — shared by the Data tab and the SQL tab. Whichever tab
// opens the sheet supplies a context describing the rows on screen, which
// column (if any) uniquely identifies a row, and where PATCH/DELETE go.
// ==========================================================================

// Context for the currently open row edit sheet, set by openRowEditSheet():
//   rows       array of row objects currently on screen (the source data)
//   idx        index into rows of the row being edited
//   dsn        DSN the target table lives in
//   table      table PATCH/DELETE requests are sent to
//   keyCol     column in `rows` whose value uniquely identifies the row
//              within `table`, or null when the row cannot be identified
//              (the sheet then opens read-only)
//   columnMeta [{name, type, …}] used only for int/float display coercion —
//              [] is fine since the server itself coerces PATCH values
//              against the column's real type
//   onSaved    called after a successful save or delete so the caller can
//              refresh its own view
let _rowEditCtx = null;

// Open the edit sheet for the Data tab row at the given index in _dataRows.
function showDataEdit(rowIdx) {
    openRowEditSheet({
        rows:       _dataRows,
        dsn:        document.getElementById('data-dsn-picker').value,
        table:      document.getElementById('data-table-picker').value,
        keyCol:     findUniqueKeyCol(),
        columnMeta: _dataColumnMeta,
        onSaved:    loadDataRows,
    }, rowIdx);
}

// Open the edit sheet for rowIdx within the given context (see _rowEditCtx above).
function openRowEditSheet(ctx, rowIdx) {
    const row = ctx.rows[rowIdx];
    if (!row) return;

    _rowEditCtx     = ctx;
    _rowEditCtx.idx = rowIdx;

    // == null (loose equality) catches both null and undefined — either
    // means the row is read-only.
    const keyCol  = ctx.keyCol;
    ...
    const metaByName = {};
    for (const col of ctx.columnMeta) metaByName[col.name] = col;
...
function checkRowEditChanged() {
    const row = _rowEditCtx.rows[_rowEditCtx.idx];
    if (!row) return;
...
async function submitRowEdit() {
    const ctx    = _rowEditCtx;
    const row    = ctx.rows[ctx.idx];
    const dsn    = ctx.dsn;
    ...
    document.querySelectorAll('#data-edit-fields .data-edit-input').forEach(input => {
        ...
        if (isNull) {
            ...
            if (isDataIntType(type)) {
                ...
                // No column metadata (e.g. a SQL tab result) or a plain string
                // column — send the raw text and let the server coerce it
                // against the column's real type.
    ...
    if (Object.keys(payload).length === 0) { hideRowEdit(); return; }
...
async function submitRowDelete() {
    const ctx    = _rowEditCtx;
    const row    = ctx.rows[ctx.idx];
    const dsn    = ctx.dsn;
    const table  = ctx.table;

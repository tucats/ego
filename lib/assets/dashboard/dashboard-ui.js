...
// Highlight the single .settings-style-option matching value inside the
// toolbar-style picker (see the "Toolbar buttons" row). A dedicated function
// rather than reusing syncSegmented(), since the cards use their own
// .settings-style-option class rather than .settings-segmented-btn.
function syncStylePicker(containerId, value) {
    document.querySelectorAll('#' + containerId + ' .settings-style-option').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.value === value);
    });
}
...
function showSettings() {
    ...
    syncStylePicker('setting-toolbar-style', getToolbarStyle());
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // "Toolbar buttons" — Icon + Text vs Icons only, applied app-wide via a
    // body class; see applyToolbarStyle() in dashboard-core.js.
    document.querySelectorAll('#setting-toolbar-style .settings-style-option').forEach(btn => {
        btn.addEventListener('click', function () {
            setToolbarStyle(this.dataset.value);
            syncStylePicker('setting-toolbar-style', this.dataset.value);
        });
    });

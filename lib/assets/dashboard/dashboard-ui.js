...
async function submitLogin() {
    ...
    try {
        ...
        // Code-tab access; data.sql is true for SQL-tab access. If none of
        // these are set, refuse to log in.
        if (!data.admin && !data.coder && !data.sql) {
            document.getElementById('login-error').textContent =
        ...
        setToken(data.token);
        setRole(data.admin, data.coder, data.sql, data.identity);
        ...
        // Open the last active tab for admins; a non-admin always lands on
        // whichever of Code/SQL their permissions unlock.
        openTab(_isAdmin ? activeTab : defaultNonAdminTab());

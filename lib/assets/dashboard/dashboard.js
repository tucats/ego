...
const _overlayDismiss = {
    ...
    'sql-build-overlay':     { hide: () => hideSqlBuild(),      dirty: false },
};
...
// Open the Build wizard. Requires a DSN to be selected in the SQL tab.
async function showSqlBuild() {
    ...
    document.getElementById('sql-build-type').value = 'SELECT';
...
// Rebuild the wizard body whenever the statement type selector changes.
// This is an async function because it needs to await two network calls:
// first to load the table list, then to load the selected table's columns.
// Both calls are awaited so the wizard body is fully populated before
// buildSqlPreview() runs for the first time.
...
// preview <pre>. Handles CREATE, INSERT, UPDATE, DELETE, and SELECT. Also enables

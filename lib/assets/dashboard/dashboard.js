...
const ADMIN_ONLY_TABS = ['memory', 'users', 'dsns', 'tables', 'data', 'log'];
...
function setShowConsole(value) {
    setCookie(COOKIE_SHOW_CONSOLE, value ? '1' : '0', PREF_MAX_AGE);
}
...
async function loadDsns() {
    ...
    try {
        ...
        for (const d of dsns) {
            ...
            //
            // Each row is clickable: clicking switches to the Tables tab and
            // pre-selects this DSN. The data-name attribute stores the DSN name
            // so openDsnInTables() can read it from the event target if needed.
            const safeName = escapeHtml(d.name);
            html += '<tr class="dsn-row" onclick="openDsnInTables(\'' + safeName + '\')"'
                  + ' title="Click to view tables in ' + safeName + '">'
                  + '<td>' + safeName                  + '</td>'
...
// Switch to the Tables tab with the given DSN pre-selected. Called when the
// user clicks a row in the DSNs table. _pendingTablesDsn is read by
// loadTables() to override the picker value before fetching the table list.
function openDsnInTables(name) {
    _pendingTablesDsn = name;
    openTab('tables');
}
...
function initCodeEditor() {
    ...
    const modeLabelMap = { run: '\u25B6 Run', trace: '\u{1F50E} Trace', debug: '\u{1F41B} Debug' };
    ...
    function setDebugButtonsEnabled(enabled) {
        ...
        codeDebugStepIntoBtn.disabled  = !enabled;
    ...
    async function runEditorCode() {
        ...
        // Normal run or trace mode.
        try {
            const payload = { code, session: codeSessionUUID };
            if (codeRunMode === 'trace') payload.trace = true;
    ...
    codeDebugStepIntoBtn.addEventListener('click',  () => sendDebugCommand('step into'));

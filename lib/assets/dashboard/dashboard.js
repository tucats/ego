...
function setDarkMode(value) {
    setCookie(COOKIE_DARK_MODE, value ? '1' : '0', SETTINGS_MAX_AGE);
    applyDarkMode(value);
}
...
// Code tab only. The optional identity parameter records the logged-in username.
//
...
function setRole(admin, coder, identity) {
    ...
    if (getRememberLogin()) {
        setCookie(COOKIE_ROLE, roleValue, TOKEN_MAX_AGE);
...
const _overlayDismiss = {
    ...
    'code-config-overlay':   { hide: () => hideCodeConfig(),    dirty: false },
};
...
// Close the Code tab's Configure sheet. (Opened by showCodeConfig(), defined
// inside initCodeEditor() since it needs access to the Code tab's own toggle
// state; closing needs no such state, so it can be a plain top-level function
// callable from both the sheet's Close button and _overlayDismiss below.)
function hideCodeConfig() {
    document.getElementById('code-config-overlay').style.display = 'none';
}
...
async function submitLoggerConfig() {
    ...
    if (!keepChanged && !loggerChanged) {
        setCookie(COOKIE_LOG_TAIL, String(tailVal), SETTINGS_MAX_AGE);
        hideLoggerConfig();
        return;
    }
...
function initCodeEditor() {
    ...
    const codeConfigBtn          = document.getElementById('code-config-btn');
    const codeConfigTrace        = document.getElementById('code-config-trace');
    const codeConfigFormat       = document.getElementById('code-config-format');
    const codeConfigConsole      = document.getElementById('code-config-console');
    ...
    function applyTraceToggle(enabled) {
        ...
        codeConfigTrace.checked = enabled;
    }
    ...
    // across sessions (unlike Trace, which is a one-shot per-run flag), so it
    // uses the getCodeFormat/setCodeFormat cookie helpers instead of a plain
    // in-memory variable.
    let codeFormatEnabled = getCodeFormat();

    function applyFormatToggle(enabled) {
        codeFormatEnabled = enabled;
        codeConfigFormat.checked = enabled;
    }

    applyFormatToggle(codeFormatEnabled);
    ...
    function applyConsoleVisible(visible) {
        codeUI.classList.toggle('hide-console', !visible);
        codeConfigConsole.checked = visible;
    }
    ...
    // -----------------------------------------------------------------------
    // Code Configure sheet
    //
    // Opened from the toolbar's "Configure..." button. Holds the Trace,
    // Format, and Console toggles as real checkbox switches (matching the
    // main Settings sheet's style) instead of the toolbar's old dedicated
    // toggle buttons -- new Code tab settings can be added here as additional
    // .settings-row blocks without further cluttering the toolbar.
    // -----------------------------------------------------------------------

    function showCodeConfig() {
        codeConfigTrace.checked   = codeTraceEnabled;
        codeConfigFormat.checked  = codeFormatEnabled;
        codeConfigConsole.checked = !codeUI.classList.contains('hide-console');
        document.getElementById('code-config-overlay').style.display = 'flex';
    }

    codeConfigBtn.addEventListener('click', showCodeConfig);

    codeConfigTrace.addEventListener('change', () => {
        applyTraceToggle(codeConfigTrace.checked);
    });

    codeConfigFormat.addEventListener('change', () => {
        setCodeFormat(codeConfigFormat.checked);
        applyFormatToggle(codeConfigFormat.checked);
    });

    codeConfigConsole.addEventListener('change', () => {
        setShowConsole(codeConfigConsole.checked);
        applyConsoleVisible(codeConfigConsole.checked);
    });

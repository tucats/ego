...
function initCodeEditor() {
    ...
    const codeFormatToggleBtn    = document.getElementById('code-format-toggle');
    const codeFormatIndicator    = document.getElementById('code-format-toggle-indicator');
    ...
    function applyFormatToggle(enabled) {
        ...
        codeFormatToggleBtn.classList.toggle('active', enabled);
        codeFormatIndicator.textContent = enabled ? '■' : '▣';
    }

    applyFormatToggle(codeFormatEnabled);

    codeFormatToggleBtn.addEventListener('click', () => {
        const enabled = !codeFormatEnabled;
        setCodeFormat(enabled);
        applyFormatToggle(enabled);
    });

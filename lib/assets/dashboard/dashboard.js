...
//
// The role cookie's lifetime always mirrors the token cookie's: both persist
// across restarts when "remember login" is on, and both are session-scoped
// (and cleaned up immediately) when it's off. They used to have independent
// lifetimes (role was always session-only) which could desync after a
// restart -- token still valid, role gone -- leaving a "logged in" user with
// no role and therefore no visible tabs.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // deletes any stale token/role cookie when turned off; when turned on
    // while already logged in, write the current in-memory session out too.
...
(async function () {
    ...
    // people back in as that stale identity. If we find a leftover token or
    // role cookie while the preference is off, discard them now instead of
    // ever reading them.
    if (!getRememberLogin()) {
        deleteCookie(COOKIE_TOKEN);
        deleteCookie(COOKIE_ROLE);

...
async function loadData() {
    ...
    const previousDsn = dsnPicker.value;
...
async function loadDataTables() {
    ...
    const previousTable = tablePicker.value;
...
function renderDataRows() {
    ...
    // Collect column names (union across all rows, preserving order), excluding _row_id_.
    ...
    // Apply visibility filter — a column is visible unless explicitly set to false.
...
function showDataEdit(rowIdx) {
    ...
    // so every field is shown regardless of current visibility.
    const colSet = new Set();

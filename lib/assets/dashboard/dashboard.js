...
function showEditUserSheet(name, perms) {
    ...
    if (passkeyBtn) {
        passkeyBtn.style.display = (_passkeysEnabled && ownAccount && window.PublicKeyCredential) ? '' : 'none';
    }
    ...
    if (clearPasskeyBtn) {
        clearPasskeyBtn.style.display = (_passkeysEnabled && (_isAdmin || ownAccount)) ? '' : 'none';
    }
...
function applyPasskeyLoginUI() {
    const show = _passkeysEnabled && !!window.PublicKeyCredential;

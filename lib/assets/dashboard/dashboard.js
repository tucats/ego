...
// Data, Log, and Code.
//
// THE DASHBOARD (HTML, CSS, AND JAVASCRIPT) ARE ENTIRELY WRITTEN BY CLAUDE
// CODE. The dashboard uses api endpoints in the Ego server that were written
// by humans, as is the rest of the Ego server.
//
// ==========================================================================
// Cookie helpers
//
...
async function loadUsers() {
    ...
    try {
        ...
        let html = '<table><thead><tr><th>User</th><th>ID</th><th>Permissions</th><th>Passkeys</th></tr></thead><tbody>';
        ...
        const perms    = Array.isArray(u.permissions) ? u.permissions.join(', ') : (u.permissions || '');
            ...
            // data-name, data-perms, and data-passkeys carry the row's values into the
            // click handler without needing a global variable. escapeHtml() is used both
            // for display and for safely encoding the attribute values.
            html += '<tr data-name="' + escapeHtml(u.name) + '" data-perms="' + escapeHtml(perms) + '" data-passkeys="' + escapeHtml(passkeys) + '">'
        ...
        container.querySelectorAll('tbody tr').forEach(row => {
            row.addEventListener('click', () => {
                showEditUserSheet(row.dataset.name, row.dataset.perms, row.dataset.passkeys);
            });
        });
    } catch (e) {
...
// Load the SQL tab — populates the DSN picker and refreshes the highlight layer.
...
// Execute the SQL commands — PUT lines as a JSON array to the server,
// then render the result as a table (or a row-count message).
...
async function submitSql() {
    ...
    // Split into lines, then drop trailing blank lines.
    const lines = text.split('\n');
    while (lines.length > 0 && lines[lines.length - 1].trim() === '') lines.pop();

    document.getElementById('sql-status').innerHTML  = '<span style="color:#666;">Running\u2026</span>';
    document.getElementById('sql-results').innerHTML = '';
...
// Column names are derived from the keys of the first row, with _row_id_ hidden.
function renderSqlResults(rows) {
    const cols = Object.keys(rows[0]).filter(k => k !== '_row_id_');
...
function showEditUserSheet(name, perms, passkeys) {
...
// Wire the SQL editor highlight layer after the DOM is fully loaded.
initSqlEditor();

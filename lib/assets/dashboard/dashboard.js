...
function showSettings() {
    ...
    document.getElementById('setting-trace').checked          = codeTraceEnabled;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // "Trace" — one-shot in-memory flag read by runEditorCode()/sendDebugCommand()
    // in initCodeEditor(); no persistence, no other side effect to apply.
    document.getElementById('setting-trace').addEventListener('change', function () {
        codeTraceEnabled = this.checked;
    });
...
// Trace/Format/Console settings live in the main Settings sheet (hamburger
...
// Trace toggle — when on, payload.trace = true is sent with each run. A
// one-shot in-memory flag (not persisted); reset to off on page reload.
...
function initCodeEditor() {
    ...
    // Console visibility, Trace, and Format are all now set from the main
    // Settings sheet (hamburger menu) -- see applyConsoleVisible() and the
    // setting-trace/setting-format/setting-console wiring near showSettings().

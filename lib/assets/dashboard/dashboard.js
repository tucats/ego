...
async function loadMemory() {
    ...
    try {
        ...
        let memHtml = '<table class="stat-grid"><thead><tr><th colspan="5">Metrics</th></tr></thead><tbody>';
        memHtml += '<tr>' + pair('Uptime',             fmtUptime(_serverStartTime))       + '<td class="stat-sep"></td>' + pair('Application Memory', fmtBytes(d.system))   + '</tr>';
        memHtml += '<tr>' + pair('Requests Processed', d.server.session.toLocaleString()) + '<td class="stat-sep"></td>' + pair('Heap Memory',         fmtBytes(d.current)) + '</tr>';
        memHtml += '<tr>' + pair('Objects In Use',     d.objects.toLocaleString())        + '<td class="stat-sep"></td>' + pair('Stack Memory',         fmtBytes(d.stack))   + '</tr>';
        memHtml += '<tr>' + pair('GC Cycles Run',      d.gc.toLocaleString())             + '<td class="stat-sep"></td>' + emptyPair()                                       + '</tr>';
        memHtml += '</tbody></table>';
        ...
        cacheHtml += '<tr>' + pair('DSN Entries',         d.dsnCount)                    + '<td class="stat-sep"></td>' + pair('Cached Services',    d.serviceCount)               + '<td class="stat-sep"></td>' + pair('Authorizations', d.authorizationCount) + '</tr>';
        cacheHtml += '<tr>' + pair('Schema Entries',      d.schemaCount)                 + '<td class="stat-sep"></td>' + pair('Service Cache Size', d.serviceSize + '&nbsp;items') + '<td class="stat-sep"></td>' + pair('Tokens',         d.tokenCount)         + '</tr>';
        cacheHtml += '<tr>' + pair('Code Run Sessions',   d.runCount)                    + '<td class="stat-sep"></td>' + pair('Cached Assets',      d.assetCount)                 + '<td class="stat-sep"></td>' + pair('Blacklist',       d.blacklistCount)     + '</tr>';
        cacheHtml += '<tr>' + pair('Code Debug Sessions', d.debugCount)                  + '<td class="stat-sep"></td>' + pair('Asset Cache Size',   fmtBytes(d.assetSize))        + '<td class="stat-sep"></td>' + emptyPair()                                   + '</tr>';

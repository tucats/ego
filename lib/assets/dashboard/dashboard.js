...
async function loadMemory() {
    ...
    // ---- Memory stats (2-column compact grid) --------------------------------
    const memContainer = document.getElementById('memory-content');
    try {
        const res = await apiFetch('/admin/memory');
        const d   = await res.json();

        let html = '<table class="stat-grid"><thead><tr><th colspan="5">Metrics</th></tr></thead><tbody>';
        html += '<tr>' + pair('Uptime',             fmtUptime(_serverStartTime))          + '<td class="stat-sep"></td>' + pair('Application Memory', fmtBytes(d.system))   + '</tr>';
        html += '<tr>' + pair('Requests Processed', d.server.session.toLocaleString())    + '<td class="stat-sep"></td>' + pair('Heap Memory',         fmtBytes(d.current)) + '</tr>';
        html += '<tr>' + pair('Objects In Use',     d.objects.toLocaleString())           + '<td class="stat-sep"></td>' + pair('Stack Memory',         fmtBytes(d.stack))   + '</tr>';
        html += '<tr>' + pair('GC Cycles Run',      d.gc.toLocaleString())                + '<td class="stat-sep"></td>' + emptyPair()                                       + '</tr>';
        html += '</tbody></table>';
    ...
    // ---- Cache stats (3-column compact grid) --------------------------------
    const cacheContainer = document.getElementById('caches-content');
    try {
        const res  = await apiFetch('/admin/caches');
        const data = await res.json();

        let html = '<table class="stat-grid"><thead><tr><th colspan="8">Cache Status</th></tr></thead><tbody>';
        html += '<tr>' + pair('DSN Entries',         data.dsnCount)                    + '<td class="stat-sep"></td>' + pair('Cached Services',    data.serviceCount)               + '<td class="stat-sep"></td>' + pair('Authorizations', data.authorizationCount) + '</tr>';
        html += '<tr>' + pair('Schema Entries',      data.schemaCount)                 + '<td class="stat-sep"></td>' + pair('Service Cache Size', data.serviceSize + '&nbsp;items') + '<td class="stat-sep"></td>' + pair('Tokens',         data.tokenCount)         + '</tr>';
        html += '<tr>' + pair('Code Run Sessions',   data.runCount)                    + '<td class="stat-sep"></td>' + pair('Cached Assets',      data.assetCount)                 + '<td class="stat-sep"></td>' + pair('Blacklist',       data.blacklistCount)     + '</tr>';
        html += '<tr>' + pair('Code Debug Sessions', data.debugCount)                  + '<td class="stat-sep"></td>' + pair('Asset Cache Size',   fmtBytes(data.assetSize))        + '<td class="stat-sep"></td>' + emptyPair()                                     + '</tr>';
        html += '</tbody></table>';

        const items = data.items || [];
        if (items.length > 0) {
            html += '<hr class="stat-divider"><table><thead><tr>'
...
async function submitSql() {
    ...
    try {
        ...
        if (data.rows && data.rows.length > 0) {
            ...
            document.getElementById('sql-status').innerHTML =
                '<span style="color:#2a7; font-size:0.9rem;">' + count

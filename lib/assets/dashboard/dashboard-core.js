...
// Tab IDs that are only visible to admin users, with two exceptions: 'code'
// is not in this list at all (always visible; access to it is instead
// enforced by forcing a non-coder to land elsewhere -- see openTab() call
// sites), and 'sql' is in this list but is also shown to an ego.sql holder
// (see the special case in applyTabVisibility below), the same way 'code'
// is shown to an ego.code holder.
...
// Encode the current in-memory role flags (_isAdmin/_isCoder/_isSql) as the
// COOKIE_ROLE string value: 'admin' for full access, otherwise a
// comma-separated list of whichever of 'coder'/'sql' apply (e.g. 'coder',
// 'sql', or 'coder,sql'), or '' for none. Shared by setRole() and the
...
// Store the user's role flags. 'admin' means full access; 'coder' unlocks
// the Code tab, 'sql' unlocks the SQL tab -- a non-admin user can hold
// either, both, or (if login was even allowed) neither. The optional
// identity parameter records the logged-in username.
//
...
function setRole(admin, coder, sqlUser, identity) {
    _isAdmin = !!admin;
    _isCoder = !!coder;
    _isSql = !!sqlUser;
...
// A coder-only user sees only the Code tab (which is not in ADMIN_ONLY_TABS
// at all -- see its definition above); a sql-only user additionally sees the
// SQL tab; an admin user sees all tabs. This must be called after every
// login and after every page-load token restore.
function applyTabVisibility() {
    ADMIN_ONLY_TABS.forEach(tabId => {
        ...
        if (btn) {
            const visible = _isAdmin || (tabId === 'sql' && _isSql);
            btn.style.display = visible ? '' : 'none';
        }
    });
}

// Pick the tab a non-admin user should land on: Code if they hold ego.code
// (preferred when they hold both, matching the historical "coders land on
// Code" behavior), otherwise SQL if they hold ego.sql. Login is refused
// (see submitLogin()/webauthn finish handler) unless at least one role flag
// is set, so this always has something to return in practice; 'code' is a
// defensive fallback, not a reachable default.
...
const _overlayDismiss = {
    ...
    'config-item-overlay':   { hide: () => hideConfigItemDetail(), dirty: false },

...
// Whether the server is configured for AI-assisted SQL generation
// (ego.server.ai.model is non-empty). Loaded once at startup from
// /services/admin/features -- a credential-free probe endpoint, since the
// SQL tab's Generate button must work for any ego.sql user, not just
// ego.root (GET/POST /admin/config, which also has this setting, is
// root-only). Defaults to false so the Generate button stays hidden until
// the server confirms AI generation is available.
let _aiGenerateEnabled = false;
...
const _overlayDismiss = {
    ...
    'data-edit-overlay':     { hide: () => hideRowEdit(),       dirty: true  },
    ...
    // dirty: false here because hideTaskDetail() itself performs the dirty
    // check (see dashboard-admin.js) -- unlike every other sheet, its Close
    // button needs the same confirm-before-discard prompt a backdrop click
    // gets, so the check lives in the shared hide function both paths call,
    // not in this table (which would double-prompt on a backdrop click).
    'task-detail-overlay':   { hide: () => hideTaskDetail(),    dirty: false },

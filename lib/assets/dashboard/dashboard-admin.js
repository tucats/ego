...
// Open the task detail sheet for the given task ID. Shows every field GET
// /admin/tasks reports (defs.TaskStatus) as a two-column table, and offers
// Run/Disable actions against /admin/tasks/{id}. There is no GET
// /admin/tasks/{id} endpoint, so this re-fetches the whole list (same as
// showDsnDetail() does for dsns) and picks out the one row it needs -- which
// also means the sheet always reflects the task's latest run.
async function showTaskDetail(id) {
    ...
    try {
        ...
        if (!task) {
            document.getElementById('task-detail-error').textContent = 'Task not found.';
            content.innerHTML = '';
            return;
        }
        ...
        // Render every defs.TaskStatus field as a two-column key/value table,
        // matching showDsnDetail()'s attribute-table layout.
        const rows = [
            ['Task',        task.task],
            ['ID',          task.id],
        ...
        content.innerHTML = '<table><thead><tr><th>Attribute</th><th>Value</th></tr></thead><tbody>'

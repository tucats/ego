...
// POST /dsns/{dsn}/generate.
// ==========================================================================

// AbortController for the in-flight /generate request, if any. Set by
...
// Aborts a still-in-flight /generate request, if any, so its response is
...
async function submitSqlGenerate() {
    ...
    try {
        const token = getToken();
        const res = await fetch('/dsns/' + encodeURIComponent(dsn) + '/generate', {
            method:  'POST',
            headers: {
                'Content-Type':  'application/json',
                'Authorization': token ? 'Bearer ' + token : '',

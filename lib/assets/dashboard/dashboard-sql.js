...
// Rows from the last successful SELECT, and the unique key column / target
// table for editing them, if any. Set by submitSql(); consumed by
// renderSqlResults() and showSqlRowEdit(). _sqlPKey/_sqlPTable are '' when
// the query's result cannot be safely traced back to a single row of a
// single table (a join, a UNION, an aliased key column, etc.) — in that
// case rows are still clickable but open read-only, same as an
// unidentifiable row on the Data tab.
let _sqlRows   = [];
let _sqlPKey   = '';
let _sqlPTable = '';
...
async function submitSql() {
    ...
    try {
        ...
        if (data.rows && data.rows.length > 0) {
            document.getElementById('sql-status').innerHTML = '';
            // pkey/ptable are only set when the RowSet came from a plain,
            // single-table SELECT and pkey names an unaliased unique column —
            // that is exactly when a row can be safely PATCHed/DELETEd.
            _sqlRows   = data.rows;
            _sqlPKey   = data.pkey   || '';
            _sqlPTable = data.ptable || '';
...
// Every row is clickable and opens the shared row-edit sheet (see
// showSqlRowEdit) — editable when _sqlPKey/_sqlPTable identify it, read-only
// otherwise.
function renderSqlResults(rows, columns) {
    ...
    for (const col of cols) {
        html += '<th>' + escapeHtml(col) + (col === _sqlPKey ? ' \u{1F511}' : '') + '</th>';
    }
    html += '</tr></thead><tbody>';
    ...
    container.querySelectorAll('tbody tr').forEach(tr => {
        tr.addEventListener('click', () => showSqlRowEdit(parseInt(tr.dataset.rowIdx, 10)));
    });
}

// Open the row-edit sheet for a SQL result row. Editable only when the last
// query reported both a pkey (an unaliased unique column) and a ptable — a
// join, a UNION, or an aliased key column leaves one or both empty, and the
// sheet then opens read-only via the same path a keyless Data tab row takes.
function showSqlRowEdit(rowIdx) {
    openRowEditSheet({
        rows:       _sqlRows,
        dsn:        document.getElementById('sql-dsn-picker').value,
        table:      _sqlPTable,
        keyCol:     _sqlPKey || null,
        columnMeta: [],
        onSaved:    submitSql,
    }, rowIdx);
}
...
// Shows or hides the Generate toolbar button to match _aiGenerateEnabled
// (see dashboard-core.js), which loadFeaturesConfig() sets at startup from
// GET /services/admin/features. Hidden by default until that resolves, since
// the server may not have ego.server.ai.model configured.
function applySqlGenerateButtonVisibility() {
    const btn     = document.getElementById('sql-generate-btn');
    const spacer  = document.getElementById('sql-generate-spacer');
    const display = _aiGenerateEnabled ? 'inline-block' : 'none';
    if (btn)    btn.style.display    = _aiGenerateEnabled ? '' : 'none';
    if (spacer) spacer.style.display = display;
}

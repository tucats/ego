...
func makeClientST(t *testing.T, dbPath string) (*symbols.SymbolTable, *data.Struct) {
	...
	connStr := "sqlite3://" + dbPath
	args := data.NewList(connStr)

	result, err := newConnection(s, args)
	if err != nil {
		t.Fatalf("makeClientST: newConnection failed: %v", err)
	}

	clientStruct, ok := result.(*data.Struct)
	if !ok {
...
func TestNewConnection_ValidSQLite3URL(t *testing.T) {
...
func TestNewConnection_ConnectionStringStored(t *testing.T) {
	...
	clientStruct := result.(*data.Struct)

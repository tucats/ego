...
func blockPrint(s *symbols.SymbolTable, args data.List) (interface{}, error) {
	initFonts()
	...
	fontName := "standard"
	if args.Len() > 1 {
		fontName = data.String(args.Get(1))
	}
...
func blockFonts(s *symbols.SymbolTable, args data.List) (interface{}, error) {
	initFonts()

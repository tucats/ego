...
func ServiceHandler(session *server.Session, w http.ResponseWriter, r *http.Request) int {
	...
	for k, v := range session.URLParts {
		ui.Log(ui.RestLogger, "[%d]  URL part %s = %v", session.ID, k, v)
	...
	serviceCode, tokens, compilerInstance, err := getCachedService(session.ID, endpoint, symbolTable)
	if err != nil {
	...
	if err != nil {
		...
		ui.Log(ui.InfoLogger, "[%d] STATUS %d", session.ID, status)
	...
	if found && responseObject != nil {
		...
		ui.Log(ui.InfoLogger, "[%d] STATUS %d, sending JSON response", session.ID, status)
	} else {
		...
		ui.Log(ui.InfoLogger, "[%d] STATUS %d, sending TEXT response", session.ID, status)
	}

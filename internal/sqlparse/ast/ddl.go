...
// ColumnIdentity is PostgreSQL's column-level "GENERATED { ALWAYS | BY
// DEFAULT } AS IDENTITY [(seq_options)]" — the SQL-standard generated-key
// constraint. It is also what the legacy SERIAL/BIGSERIAL/SMALLSERIAL
// pseudo-types desugar to during parsing (see parseColumnDef in ddl.go),
// and what sqlite3's PRIMARY KEY AUTOINCREMENT (ColumnPrimaryKey.
// AutoIncrement) rewrites to when a statement is normalized toward
// PostgreSQL (see Rewrite in rewrite.go). Always distinguishes "GENERATED
// ALWAYS" (an explicit value in an INSERT is rejected) from "GENERATED BY
// DEFAULT" (an explicit value is honored) — sqlite3's AUTOINCREMENT and the
// SERIAL family both match "BY DEFAULT" semantics, so Always is false in
// both of those cases. Any parenthesized sequence-option list following
// IDENTITY is accepted but not retained — this package's DDL grammar
// otherwise never models sequence generator options.
type ColumnIdentity struct {
	BaseNode
	Always bool
}

func (n *ColumnIdentity) Kind() Kind       { return KindColumnIdentity }
func (n *ColumnIdentity) Children() []Node { return nil }
func (n *ColumnIdentity) String() string   { return "ColumnIdentity" }

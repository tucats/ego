...
const (
	// SQLStringType uses TEXT rather than "char varying" so that CREATE TABLE
	// statements generated by the resources package work identically on both
	// SQLite (which only recognizes TEXT) and PostgreSQL (which accepts TEXT as
	// an alias for varchar with no length limit).
	SQLStringType = "TEXT"
	SQLIntType    = "integer"
	SQLBoolType   = "boolean"
	SQLFloatType  = "float"

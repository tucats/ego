...
type LoggingItem struct {
	...
	Loggers map[string]bool `json:"loggers,omitempty" validate:"enum=ai|auth|db|internal|resources|rest|server|sql|tables|valid|app|asset|bytecode|cache|child|cli|compiler|debug|goroutine|info|optimizer|packages|route|services|stats|symbols|tasks|tokenizer|trace|user"`
}

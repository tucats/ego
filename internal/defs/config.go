...
const (
	...
	// DashboardInactivityTimeoutSetting is a Go duration string (e.g. "15m",
	// "1h") controlling how long the web dashboard waits for user activity
	// before automatically signing the user out. Returned to the dashboard as
	// part of the logon response so it can apply the same value locally
	// instead of using its own hard-coded default. Defaults to "15m" when unset.
	DashboardInactivityTimeoutSetting = ServerKeyPrefix + "dashboard.inactivity"
...
var ValidSettings map[string]bool = map[string]bool{
	...
	DashboardInactivityTimeoutSetting: true,

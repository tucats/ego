...
| bytecode (alias `disasm`) | | If present, prints a disassembly of the bytecode generated for this block once it compiles successfully. There is no option value, the option is true if `bytecode` (or `disasm`) is present. See below. |
...
#### Viewing generated bytecode with `bytecode` (or `disasm`)
...
generated for a block of code. Adding the `bytecode` keyword (or its alias,
`disasm`) to `@compile` prints a disassembly listing of the block once it
finishes compiling successfully:

```go
@compile block bytecode {
    x := 1
    fmt.Println(x)
}
```

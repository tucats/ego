...
- **Cluster Issues** (`CLUSTER-1`): Documents defects in multi-node cluster coordination. CLUSTER-1 is a feedback loop in cache invalidation: the inbound-flush handler called the same `caches.Purge` that fires the broadcast hook, so every node answered each notification it received with one of its own, unbounded. Fixed structurally with a non-broadcasting `caches.PurgeLocal`, plus a hop-count circuit breaker as defense in depth.
